...
func RandomStringWithPrefix(prefix string, size int, charset []rune) string {
	return fmt.Sprintf("%s%s", prefix, RandomString(size, charset))
}
...
func HashString(payload interface{}) (string, error) {
	h := sha256.New()
